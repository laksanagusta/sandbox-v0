...
import { Search, User, ArrowUpDown, ArrowUp, ArrowDown, Building, Phone, Check, X, List } from "lucide-react";
...
export function PendingUserTable({ className = "" }: PendingUserTableProps) {
  ...
  const confirmAction = (user: User, type: "approve" | "reject") => {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (actionType === "approve") {
        await apiClient.approveUser(selectedUser.id);
        toast({
          title: "Sukses",
          description: `User ${selectedUser.username} berhasil disetujui`,
        });
      } else {
        await apiClient.rejectUser(selectedUser.id);
        toast({
          title: "Sukses",
          description: `User ${selectedUser.username} berhasil ditolak`,
        });
      }
      fetchUsers();
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: `Gagal ${actionType === "approve" ? "menyetujui" : "menolak"} user`,
        variant: "destructive",
      });
    } finally {
    ...
    <div className={`bg-white flex flex-col h-full ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center px-6 py-2 border-b space-y-4 sm:space-y-0 min-h-[52px]">
        <div className="flex items-center space-x-4">
          <span className="text-sm font-semibold text-gray-900">Pending Users</span>
      ...
      {!loading && (
        <div className="sticky bottom-0 bg-white z-10 flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t gap-4 mt-auto">
          <div className="text-xs text-muted-foreground order-2 sm:order-1">
            Showing <strong>{users.length}</strong> of <strong>{pagination.total_count}</strong> requests
          </div>
      ...
      <AlertDialog open={!!selectedUser} onOpenChange={(open) => !open && setSelectedUser(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Apakah Anda yakin ingin {actionType === "approve" ? "menyetujui" : "menolak"} user{" "}
              <span className="font-semibold">{selectedUser?.username}</span>?
              {actionType === "reject" && " User akan dihapus dari sistem."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {actionLoading ? "Loading..." : actionType === "approve" ? "Setujui" : "Tolak"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
